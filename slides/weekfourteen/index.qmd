...
- **No time limit**: Can use $G$ multiple times, any running time
...
- {{< iconify fa6-solid book >}} **Formal notation**: $F \leq_P G$ means "F
polyreduces to G"
...
- {{< iconify fa6-solid arrow-right >}} **Interpretation**: If G is easy, then
F is easy
...
{{< iconify fa6-solid gears >}} **Three conditions work together**: they
ensure that solving G efficiently lets us solve F efficiently. Proofgrammers
must verify all three conditions in every proof!
...
- **Transformation**: Function $R$ converts $F$ instances to $G$ instances
...
- {{< iconify fa6-solid box >}} **Input**: Weights $w_1, w_2, \ldots, w_m$ and
thresholds $L$ (low) and $H$ (high)
...
- {{< iconify fa6-solid check >}} **Example**: Weights "12 4 6 24 4 16; 20;
27" → solution "4 6 16" (total = 26)
...
**Quick PolyCheck reminder**: A problem is in PolyCheck if we can verify a
proposed solution in polynomial time, even if finding that solution might be
hard. PACKING fits perfectly: given a subset, we just sum the weights and check
if $L \leq \text{sum} \leq H$ in $O(n)$ time!
...
- **Multiple calls**: Can invoke $G$ subroutine many times
...
- **Single transformation**: Map once from $F$ to $G$
- **Fixed mapping**: Cannot adapt based on $G$'s answer
...
negative instance of $F$ maps to a negative instance of $G$
