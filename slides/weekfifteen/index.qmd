...
# NP-completeness proves most hard problems are equally hard!

::: {.fragment .boxed-content style="font-size: 0.875em;"}

{{< iconify fa6-solid trophy >}} **NP-completeness** helps with learning
objectives **CS-204-1** and **CS-204-4**. We will define a profound class
of problems that are **equally hard** and believed to be **intractable**. This
chapter introduces the most famous open problem in computer science: **P
versus NP**. **As proofgrammers, let's explore the hardest problems in NP and
understand why they all have the same fundamental difficulty**! Let's dive in!
...
**Key relationships**: We know P $\subseteq$ NP $\subseteq$ Expo, but are
these proper subsets? This is the most famous open question in computer
science!
...
- {{< iconify fa6-solid trophy >}} **Million dollar prize**: Clay
Mathematics Institute offers $1M
- {{< iconify fa6-solid microscope >}} **What it means**: Can every
problem with polytime verification be solved in polytime?
- {{< iconify fa6-solid lightbulb >}} **Current belief**: P $\neq$ NP
(verification is easier than solving)
- {{< iconify fa6-solid rocket >}} **No proof yet**: Despite decades of
effort by brilliant researchers
...
- **Green region**: This is where NP-complete problems live (if P $\neq$
NP)
...
## Understanding P versus NP visually

![](14-np-completeness_2.png)

::: {.fragment style="font-size: 0.85em;"}

- **Left diagram**: If P = NP, the classes are identical
- **Right diagram**: If P $\neq$ NP, there's a gap between them
- **Question mark**: We don't know which diagram is correct!

:::

## Two scenarios for P versus NP

![](14-np-completeness_3.png)

::: {.fragment style="font-size: 0.85em;"}

- **Scenario 1**: P = NP (unlikely but possible)
- **Scenario 2**: P $\neq$ NP (widely believed)
- **Importance**: This affects cryptography, optimization, and more
...
## Computational interpretation with trees
...
- **P $\neq$ NP would mean**: Parallel search provides exponential
speedup
...
- **Alternative view**: Shows the relationship between P and NP
- **Key insight**: NP contains problems easily verified but hard to solve
- **Proofgrammers**: Must understand both views to grasp P versus NP

:::

# We already know many polyequivalent problems!
...
## Summary diagram for decision problems only
...
## Summary including nondecision problems
...
- **Proofgrammers**: Should understand both decision and nondecision
views
...
**P = NP would be revolutionary but is considered unlikely**: the
asymmetry between solving and verification seems fundamental. Most
cryptography depends on this asymmetry!

:::

# Proving CircuitSAT is NP-complete

::: {.incremental style="margin-top: -0.5em; font-size: 0.85em;"}

- {{< iconify fa6-solid trophy >}} **Next goal**: Prove CircuitSAT is
NP-complete
- {{< iconify fa6-solid lightbulb >}} **Strategy**: Show all NP problems
reduce to it
- {{< iconify fa6-solid microchip >}} **Key fact**: Polytime programs
can be implemented by polysized circuits
- {{< iconify fa6-solid link >}} **Connection**: Circuits naturally
encode computation
- {{< iconify fa6-solid microscope >}} **Foundation**: This proof
underlies all of NP-completeness theory

:::

::: {.fragment .fade .boxed-content style="font-size: 0.8em;"}

**CircuitSAT is the bridge**: it connects abstract computation to concrete
circuits. This makes it the natural first NP-complete problem to prove!

:::

## Polytime programs as polysized circuits

::: {.incremental style="margin-top: -0.25em; font-size: 0.8em;"}

- {{< iconify fa6-solid microchip >}} **Key claim**: Programs run on
computers, which are made of circuits
- {{< iconify fa6-solid calculator >}} **Reasonable**: Circuits can
implement computation
- {{< iconify fa6-solid book >}} **Detailed proof**: See book for full
argument
- {{< iconify fa6-solid link >}} **Connection**: Polytime programs become
polysized circuits
...
![](14-np-completeness_23.png)

::: {.fragment style="font-size: 0.85em;"}

- **Circuit structure**: Shows how gates connect
- **Polynomial size**: Circuit size bounded by polynomial in input
- **Key insight**: Can encode computation as satisfiability

:::

## Second circuit diagram

![](14-np-completeness_24.png)

::: {.fragment style="font-size: 0.85em;"}

- **Another view**: Different circuit configuration
- **Same principle**: Polytime programs as polysized circuits
- **Foundation**: For proving CircuitSAT is NP-complete

:::

## Third circuit diagram

![](14-np-completeness_25.png)

::: {.fragment style="font-size: 0.85em;"}

- **Detailed structure**: Shows circuit components
- **Boolean logic**: AND, OR, NOT gates
- **Size bound**: Polynomial in input length

:::

## Fourth circuit diagram

![](14-np-completeness_26.png)

::: {.fragment style="font-size: 0.85em;"}

- **Complete example**: Full circuit for specific computation
- **Satisfiability**: Can this circuit output 1?
- **NP-complete**: This question is as hard as any NP problem

:::

## Overview of circuit reduction

![](14-np-completeness_27.png)

::: {.fragment style="font-size: 0.85em;"}

- **Reduction idea**: Convert NP problem to circuit satisfiability
- **Polynomial transformation**: Circuit size polynomial in input
- **Preserves**: Positive instances map to satisfiable circuits
- **Cook-Levin**: This completes the proof that CircuitSAT is
NP-complete
...
to circuit design to biology, the same fundamental hardness appears. This
shows the universality of computational difficulty!
